...
!function (factory) {
    ...
    var Checkboxpicker = function () {
        ...
        return _proto.init = function () {
            ...
            this.$buttons.toggleClass("disabled"), this.element.disabled ? this.$group.attr("tabindex", this.element.tabIndex) : this.$group.removeAttr("tabindex")
        }, _proto.focus = function () {
            this.$group.trigger("focus")
        }, _proto.click = function (event) {
            $(event.currentTarget).hasClass("active") && !this.options.switchAlways || this.change()
            ...
            -1 !== $.inArray(event.keyCode, this.options.toggleKeyCodes) ? (event.preventDefault(), this.change()) : 13 === event.keyCode && $(this.element.form).trigger("submit")
    ...
    return $.extend($.propHooks, {
        checked: {
            set: function (element, value) {
                ...
                data && element.checked !== value && data.change(value), oldPropHooks.checked && oldPropHooks.checked.set && oldPropHooks.checked.set(element, value)
            }
        }, disabled: {
            set: function (element, value) {
                var data = $.data(element, "bs.checkbox");
                data && element.disabled !== value && data.toggleDisabled(), oldPropHooks.disabled && oldPropHooks.disabled.set && oldPropHooks.disabled.set(element, value)
            }
        }
    }), $.fn.checkboxpicker = function (options, elements) {
        return (this instanceof $ ? this : $("string" == typeof options ? options : elements)).each((function () {
            var data = $.data(this, "bs.checkbox");
            data || (data = new Checkboxpicker(this, options), $.data(this, "bs.checkbox", data))
        }))
